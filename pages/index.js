...
export default function Home() {
  ...
  /**
   * Sets the ENS, if the current connected address has an associated ENS or else it
  ...
  /**
   * A `Provider` is needeed to interact with the blockchain - reading transactions,
   * reading balances, reading state, etc.
   *
   * A `Signer` is a special type of Provider used in a case a `write` transaction
   * needs to be made to the blockchain, which involves the connected account needing
  ...
  const connectWallet = async () => {
    try {
      ...
      // When useed for the first time, it prompts the user to connects their wallet
  ...
  */
  ...
  // In this case , whenever the calue of `walletConnected` changes - this effect will be called
  useEffect(() => {
